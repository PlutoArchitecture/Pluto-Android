...
public class BitmapCache {
                ...
                try {
                    ...
                    if (snapshot == null) {
                        ...
                        if (editor != null) {
                            ...
                            bitmap.compress(
                                    getImageType(data), mCacheParams.compressQuality, out);
                            editor.commit();
                            out.close();
                        }
                    } else {
    ...
    public Bitmap getBitmapFromMemCache(Context context, int resId) {
        if (mMemoryCache != null) {
            Bitmap memBitmap = mMemoryCache.get(resId+"");
            if (memBitmap != null) {
                return memBitmap;
            }
            memBitmap = BitmapFactory.decodeResource(context.getResources(), resId);
            if (memBitmap != null) {
				mMemoryCache.put(resId+"", memBitmap);
				return memBitmap;
			}
        }
        return null;
    ...
    public Bitmap getBitmapFromDiskCache(String data) {
    	final String key = FileNameGenerator.generator(data);
        //LogUtils.infoF("data:%s: key:%s", data, key);
        synchronized (mDiskCacheLock) {
