...
public class FinalDb extends DataManagerStub {
	...
	class SqliteDbHelper extends SQLiteOpenHelper {
		...
		private void tableAddColumns(SQLiteDatabase db, int oldVersion, int newVersion, int addColumnVersion, Class<?> tableClass, String... addColumnName) {
			if (addColumnName != null) {
				for (String columnName : addColumnName) {
					tableAddColumn(db, oldVersion, newVersion, addColumnVersion, tableClass, columnName);
				}
			}
		}
		 ...
		 * @param addColumnVersion 要升级的数据库版本，应大于数据库旧版本，小于等于新版本
		 * @param tableClass       要升级的数据表，不能为空
		 * @param addColumnName    要增加的字段，不能为空或0长度字符串
		 */
		private void tableAddColumn(SQLiteDatabase db, int oldVersion, int newVersion, int addColumnVersion, Class<?> tableClass, String addColumnName) {
			...
			if (addColumnVersion <= newVersion && addColumnVersion > oldVersion) {
				if (tableClass != null && !TextUtils.isEmpty(addColumnName)) {
					TableInfo userTableInfo = TableInfo.get(tableClass);
					String tableName = userTableInfo.getTableName();
					//打印现在数据表的字段
					LogUtils.info(getPropertyMapStr(userTableInfo));

					boolean isFieldExist = isFieldExist(db, tableName, addColumnName);
					...
					if (!isFieldExist) {
						...
						LogUtils.info(String.format("%s %s isFieldExist:%b", tableName, addColumnName, isFieldExist(db, tableName, addColumnName)));
					}
				}
			}
